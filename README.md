...
它拥有**双重核心定位**：
1. **开箱即用的大型游戏机制增强模组**：内置沉浸式「停电模式」、管理专用「维修人员模式」、5 大特色投稿职业、完整的「七宗罪」角色体系与「七美德」词条修饰符体系、淘汰玩家休息区、全功能 ModMenu 中文可视化配置中心（四分类十页面）、全服 3D MVP 结算大屏（集成 30 种动作姿势与规则定制）、智能模式与地图双阶段投票、大厅与局内环境控制、Simple Voice Chat 语音联动以及 Iris 光影白名单检测防护等。
...
- 🗳️ **智能双阶段投票（Mode & Map Voting）**：
  ...
  - **预设与介绍图支持**：支持地图预设配置与客户端地图介绍图（简介、标签、作者）高效本地缓存。
...
## ModMenu 可视化配置中心（四大分类）

模组提供了基于 ModMenu 的全中文交互式 GUI 配置界面（按 `Esc -> 选项 -> 模组 -> HabiTrain Core -> 配置` 打开），采用「左侧一级导航 + 右侧二级子标签页」设计，划分为四大核心分类共 10 个配置页面：
...
│ 一级分类     │ 二级子页面       │ 功能与配置项说明                                     │
├──────────────┼──────────────────┼──────────────────────────────────────────────────────┤
│ 1. 游戏内    │ · 小游戏         │ 管理小游戏任务池、奖励、出现条件与在各地图中的开关   │
│   (对局体验) │ · 数值平衡       │ 警长比例(分母)、小刀耐久开关、临时电力价格、致盲开关 │
│              │ · 环境           │ 对局内天气(晴/雨/雷)、时间锁定、局后环境与动态雨     │
├──────────────┼──────────────────┼──────────────────────────────────────────────────────┤
...
│   (任务角色) │ · 角色覆盖       │ v1 角色覆盖与修改补丁状态、角色参数与冲突管理        │
│              │ · 角色扩展       │ v2 Provider/Entry 状态查看、热启用/禁用与冲突裁决    │
├──────────────┼──────────────────┼──────────────────────────────────────────────────────┤
│ 4. 其他      │ · 扩展设置       │ 兼容性选项、网络同步策略、只读/OP 权限分级与全局安全 │
└──────────────┴──────────────────┴──────────────────────────────────────────────────────┘
```
...
├── client/                # ModMenu 四大分类 10 大配置页、MVP 动画、HUD、GUI、渲染与客户端 Mixin
