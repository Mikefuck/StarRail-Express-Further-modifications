...
public final class SceneProfile {
     ...
     * 显示原点（世界坐标）。
     *
     * <p>审核 S-05：返回<b>副本</b>，调用方改写它不会影响本对象，也不会绕过 revision/同步。
     * 内部热路径请用同包的 {@link #displayOriginRaw()}。</p>
     */
    public double[] getDisplayOrigin() { return Arrays.copyOf(displayOrigin, 3); }

    /** 同包内部热路径用：直接引用（只读，禁止改写）。 */
    double[] displayOriginRaw() { return displayOrigin; }
    ...
    /** 局部枢轴点（模型坐标）。审核 S-05：返回副本。内部热路径用 {@link #pivotLocalRaw()}。 */
    public double[] getPivotLocal() { return Arrays.copyOf(pivotLocal, 3); }

    /** 同包内部热路径用：直接引用（只读，禁止改写）。 */
    double[] pivotLocalRaw() { return pivotLocal; }
    ...
    /** 单位化运动方向。审核 S-05：返回副本。内部热路径用 {@link #directionRaw()}。 */
    public double[] getDirection() { return Arrays.copyOf(direction, 3); }

    /** 同包内部热路径用：直接引用（只读，禁止改写）。 */
    double[] directionRaw() { return direction; }
