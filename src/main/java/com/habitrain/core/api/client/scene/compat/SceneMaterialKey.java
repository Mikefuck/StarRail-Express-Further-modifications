...
public record SceneMaterialKey(
    ...
    /**
     * 按渲染层名映射到标准材质（{@code SOLID} / {@code CUTOUT_MIPPED} / {@code CUTOUT} /
     * {@code TRANSLUCENT}）；未知或 {@code null} 回退 {@link #SOLID}。
     *
     * <p>2.0.11 起参数由内部枚举改为字符串，使公开层不再依赖实现层类型。</p>
     */
    public static SceneMaterialKey fromLayerName(String layerName) {
        if (layerName == null) return SOLID;
