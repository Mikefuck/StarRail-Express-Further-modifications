 ...
 *
 * <p>全程只有一个画面，由服务端权威信号驱动：</p>
 * <ol>
 *   <li><b>滑入并加载</b>：投票结束后，加载面板从右向左覆盖投票页，显示地图、模式和进度。</li>
 *   <li><b>原地切换</b>：服务端确认环境就绪（{@link #confirmLaunch}）后，不再滑入第二个页面；
 *       只在当前面板内让加载内容淡出，并直接显示「对局开始」。</li>
 *   <li><b>交还</b>：SRE 进入 ACTIVE（{@link #markGameActive()}）后，开场画面整体继续向左
 *       滑出，游戏世界从画面右缘直接露出——像列车驶入下一站，无需任何刷新或黑幕。
 *       滑出完成即 {@code setScreen(destination)} 交还对局画面。</li>
 * </ol>
 *
 * <p>交还时 SRE 原版黑场与开场相机动画仍在宽限期内被屏蔽（见
 * {@code VoteLaunchFadeBlockMixin} / {@code VoteLaunchCameraBlockMixin}），
 * 因此玩家看到的是我们的画面滑走后直接露出已就绪的游戏世界，无缝衔接对局开始。</p>
 */
public final class VoteLaunchTransitionScreen extends Screen {
    ...
    // 服务端加载信息（由 MapVoteProgressPayload 更新）
    ...
    /** 服务端加载进度更新（MapVoteProgressPayload）。 */
    ...
    /** 服务端确认 SRE 与 API 的对局环境均已应用：触发右→左全屏滑入开场。 */
    ...
    public void tick() {
        ...
        // 强制停留最短加载时长，避免进度包尚未到达就瞬间滑入。
        if (!launchConfirmed && now - startedAtMillis < MIN_LOADING_MILLIS) {
            return;
        }
        if (!launchConfirmed) {
            // 已进入开局（trueStartGame）确认前的兜底：若服务端进度包迟迟不来（理论上不该发生），
            // 不要永久锁在加载视图。
        ...
        if (!exitStarted) {
            // 同一面板至少播完内容切换 + 标题时刻，才允许交还。
            long minExitAt = launchConfirmedAtMillis + CONTENT_SWITCH_MILLIS + TITLE_HOLD_MILLIS;
     ...
     * 滑动边缘：柔和羽化 + 金色核心亮线。
     *
     * @param featherDir +1 表示开场画面在边缘右侧（滑入），-1 表示在左侧（滑出）；
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        // 开局加载/转场不可手动跳过，避免 ESC 绕过服务端开局同步点。
        return true;
    }
